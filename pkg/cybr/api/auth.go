...
func (c *Client) Logon(ctx context.Context, req requests.Logon) (context.Context, *shared.ErrorResponse, error) {
	...
	ctx, response, err := httpJson.SendRequestRaw(ctx, url, "POST", "", req, c.InsecureTLS, c.Logger)
	if err != nil {
		//Check if response can be unmarshalled to ErrorResponse
		errorResponse := shared.ErrorResponse{}
		errUm := json.Unmarshal(response, &errorResponse)
		if errUm != nil {
			return ctx, nil, fmt.Errorf("Failed to authenticate to the PAS REST API. %s", errUm)
		}
		return ctx, &errorResponse, fmt.Errorf("Failed to authenticate to the REST API %s", err)
